...
def test_insufficient_balance(deployer, bob, swap, wrapped_coins, sending, receiving, wrapped_decimals):
    amount = 10 ** wrapped_decimals[sending]
    if wrapped_coins[sending] == ETH_ADDRESS:
        value = amount
    else:
        wrapped_coins[sending].transfer(bob, amount, {"from": deployer})
        value = 0
...
@pytest.mark.itercoins("sending", "receiving")
def test_min_dy_too_high(deployer, bob, swap, wrapped_coins, sending, receiving, wrapped_decimals):
    amount = 10 ** wrapped_decimals[sending]
    if wrapped_coins[sending] == ETH_ADDRESS:
        value = amount
    else:
...
@pytest.mark.skip_pool_type("eth")
