...
def test_min_dy_too_high(deployer, bob, swap, underlying_coins, underlying_decimals, sending, receiving):
    amount = 10 ** underlying_decimals[sending]

    underlying_coins[sending].transfer(bob, amount, {"from": deployer})
...
@pytest.mark.itercoins("sending", "receiving", underlying=True)
def test_insufficient_balance(deployer, bob, swap, underlying_coins, underlying_decimals, sending, receiving):
    amount = 10 ** underlying_decimals[sending]
