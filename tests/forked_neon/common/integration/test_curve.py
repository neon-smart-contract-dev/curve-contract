...
@given(
    ...
    st_seed_amount=strategy("decimal", min_value=50, max_value=56, places=1),
)
...
def test_curve_in_contract(
    ...
    st_seed_amount = int(10 ** (st_seed_amount / 10))
    ...
    for coin, decimals in zip(wrapped_coins, wrapped_decimals):
        ...
        coin.transfer(alice, amount, {"from": deployer})
    ...
    curve_model = Curve(swap.A(), balances, n_coins, rates)
