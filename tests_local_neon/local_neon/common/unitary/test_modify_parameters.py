...
pytestmark = pytest.mark.target_pool("busd", "compound", "susd", "usdt", "y")


@pytest.mark.parametrize(
    "fee,admin_fee", [(0, 0), (23, 42), (MAX_FEE, MAX_ADMIN_FEE), (1, MAX_ADMIN_FEE)]
)
def test_commit(alice, swap, fee, admin_fee):
    tx = swap.commit_new_parameters(360, fee, admin_fee, {"from": alice})
    ...
    assert swap.future_A() == 360


def test_commit_only_owner(bob, swap):
    with pytest.raises(ValueError):
        swap.commit_new_parameters(360, 23, 42, {"from": bob})


def test_commit_already_active(alice, swap):
    swap.commit_new_parameters(360, 23, 42, {"from": alice})
    with pytest.raises(ValueError):
        swap.commit_new_parameters(360, 23, 42, {"from": alice})
...
def test_commit_admin_fee_too_high(alice, swap, fee):
    with pytest.raises(ValueError):
        swap.commit_new_parameters(360, 0, fee, {"from": alice})


@pytest.mark.skip_pool("y", "usdt")
@pytest.mark.parametrize("fee", [MAX_FEE + 1, 2 ** 127, 2 ** 256 - 1])
def test_commit_fee_too_high(alice, swap, fee):
    with pytest.raises(ValueError):
        swap.commit_new_parameters(360, fee, 0, {"from": alice})
...
def test_apply(chain, alice, swap, fee, admin_fee):
    swap.commit_new_parameters(360, fee, admin_fee, {"from": alice})
...
def test_apply_only_owner(chain, alice, bob, swap):
    swap.commit_new_parameters(360, 0, 0, {"from": alice})

    with pytest.raises(ValueError):
        swap.apply_new_parameters({"from": bob})
